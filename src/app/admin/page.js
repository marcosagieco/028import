...
// --- CONFIGURACIÓN DE FIREBASE (Inicialización fuera del componente - Mandatorio) ---
...
export default function App() {
  ...
  const [saveStatus, setSaveStatus] = useState(null); // feedback visual

  // 1. Efecto de Autenticación FIRST (Mandatorio)
  useEffect(() => {
  ...
  // 2. Efecto de Datos Guardado con User (Regla 1 y 2)
  useEffect(() => {
    if (!user) return;
    ...
    }, (err) => console.error("Error Prods:", err));
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center text-[#d4af37] font-black tracking-widest uppercase">
    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans pb-20">
      ...
      <nav className="bg-[#0a0a0a] text-white p-4 sticky top-0 z-50 flex justify-between items-center border-b border-[#d4af37]/30 shadow-2xl">
        ...
        <div className="flex items-center gap-2">
          {saveStatus === 'guardando' && <span className="text-[9px] font-bold text-amber-500 uppercase animate-pulse">Guardando...</span>}
          {saveStatus === 'listo' && <span className="text-[9px] font-bold text-emerald-500 uppercase">Cambios Guardados ✔</span>}
      ...
      <div className="max-w-5xl mx-auto p-4 md:p-8 space-y-12">
        ...
        <section>
          <div className="flex items-center gap-2 mb-6 border-b pb-3 border-slate-200">
            <IconPackage />
            <h2 className="font-black uppercase text-xs text-slate-500 tracking-widest">Stock y Precios por Producto</h2>
          </div>
        ...
        <section>
          <div className="flex items-center gap-2 mb-6 border-b pb-3 border-slate-200">
            <h2 className="font-black uppercase text-xs text-slate-500 tracking-widest">Últimos Pedidos Recibidos</h2>
          </div>
          <div className="space-y-4">
            {orders.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200 text-slate-400">
                <p className="font-bold uppercase text-[10px] tracking-widest">No hay ventas registradas aún</p>
              </div>
            ) : orders.map(o => (
